...
const MOUStatusChip = ({ info }) => {
  const [isTeminationBoxShown, setIsTeminationBoxShown] = useState(false);
  const terminationStatuses = info?.row?.original?.terminationStatuses || "";
          ...
          <div className="">
           Termination End Date :{" "}
            {terminationStatuses?.terminationEndDate
              ...
              : ""}{" "}
            <span className="ml-4 bg-orange-300 p-2">
              {terminationStatuses?.terminationEndDate &&
                daysDifference(terminationStatuses?.terminationEndDate)}{" "}
              days remaining
            </span>
          </div>
        </div>
